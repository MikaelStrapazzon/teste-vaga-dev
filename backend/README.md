...
```php
use flight\Flight;require 'flight/Flight.php';
...
Flight is released under the [MIT](../index.phpm/license) license.

# Installation

1\. Download the files.

If you're using [Composer](../index.php.org/), you can run the following command:

```
composer require mikecao/flight
```

OR you can [download](../index.phpmikecao/flight/archive/master.zip) them directly 
...
```php
use flight\Flight;Flight::route('/', function(){
    echo 'hello world!';
});
```
...
```php
use flight\Flight;Flight::start();
```
...
```php
use flight\Flight;function hello(){
    echo 'hello world!';
}
...
```php
use flight\Flight;class Greeting {
...
```php
use flight\Flight;Flight::route('GET /', function(){
...
```php
use flight\Flight;Flight::route('GET|POST /', function(){
...
```php
use flight\Flight;Flight::route('/user/[0-9]+', function(){
...
```php
use flight\Flight;Flight::route('/@name/@id', function($name, $id){
    echo "hello, $name ($id)!";
});
```
...
```php
use flight\Flight;Flight::route('/@name/@id:[0-9]{3}', function($name, $id){
...
```php
use flight\Flight;Flight::route('/blog(/@year(/@month(/@day)))', function($year, $month, $day){
...
```php
use flight\Flight;Flight::route('/blog/*', function(){
...
```php
use flight\Flight;Flight::route('/user/@name', function($name){
...
```php
use flight\Flight;Flight::route('/', function($route){
...
```php
// Map your method
use flight\Flight;Flight::map('hello', function($name){
    echo "hello $name!";
});
...
```php
// Register your class
use flight\Flight;Flight::register('user', 'User');
...
use flight\Flight;Flight::register('db', 'PDO', array('mysql:host=localhost;dbname=test','user','pass'));
...
use flight\Flight;Flight::register('db', 'PDO', array('mysql:host=localhost;dbname=test','user','pass'), function($db){
...
use flight\Flight;$shared = Flight::db();
...
```php
use flight\Flight;Flight::map('notFound', function(){
...
use flight\Flight;Flight::register('router', 'MyRouter');
...
```php
use flight\Flight;Flight::before('start', function(&$params, &$output){
    // Do something
});
```
...
```php
use flight\Flight;Flight::after('start', function(&$params, &$output){
    // Do something
});
```
...
```php
// Save your variable
use flight\Flight;Flight::set('id', 123);
...
```php
use flight\Flight;if (Flight::has('id')) {
     // Do something
}
```
...
use flight\Flight;Flight::clear('id');
...
```php
use flight\Flight;Flight::set('flight.log_errors', true);
```
...
```php
use flight\Flight;Flight::render('hello.php', array('name' => 'Bob'));
```
...
```php
use flight\Flight;Flight::view()->set('name', 'Bob');
```

The variable `name` is now available across all your views. So you can simply do:

```php
use flight\Flight;Flight::render('hello');
```
...
```php
use flight\Flight;Flight::set('flight.views.path', '/path/to/views');
```
...
```php
use flight\Flight;Flight::render('header', array('heading' => 'Hello'), 'header_content');
...
```php
use flight\Flight;Flight::render('layout', array('title' => 'Home Page'));
```
...
own view class. Here's how you would use the [Smarty](../index.phpet/)
template engine for your views:

```php
// Load Smarty library
use flight\Flight;require './Smarty/libs/Smarty.class.php';
...
```php
use flight\Flight;Flight::map('render', function($template, $data){
...
```php
use flight\Flight;Flight::map('error', function(Exception $ex){
    // Handle error
...
```php
use flight\Flight;Flight::redirect('/new/location');
```
...
```php
use flight\Flight;Flight::redirect('/new/location', 401);
```
...
```php
use flight\Flight;$request = Flight::request();
```
...
```php
use flight\Flight;$id = Flight::request()->query['id'];
```
...
```php
use flight\Flight;$body = Flight::request()->getBody();
```
...
```php
use flight\Flight;$id = Flight::request()->data->id;
```
...
```php
use flight\Flight;Flight::route('/news', function(){
...
```php
use flight\Flight;Flight::halt();
```

You can also specify an optional `HTTP` status code and message:

```php
use flight\Flight;Flight::halt(200, 'Be right back...');
```
...
```php
use flight\Flight;Flight::json(array('id' => 123));
```
...
```php
use flight\Flight;Flight::jsonp(array('id' => 123), 'q');
```
...
```php
use flight\Flight;Flight::map($name, $callback) // Creates a custom framework method.
...
```php
use flight\Flight;Flight::start() // Starts the framework.
